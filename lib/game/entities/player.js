        ...
        isLeaping: function() {
            ...
            if ( ! this.leaping ) {
                if ( ig.input.released('click') ) {
                    ...
                    //console.log('x dist:', adjacent);
                    //console.log('y dist:', opposite);
                    //console.log('distance:', hypotenuse);
            ...
            if ( ! this.leaping ) {
                if ( ig.input.released('click') ) {
                    ...
                    var d = Math.abs( this.target.x - this.origin.x );
                    var g = ig.game.gravity;
                    var yo = this.target.y - this.origin.y;
                    
                    var numerator = ( ( d * d ) * g );
                    var denominator = 2 * ( Math.cos(theta) * Math.cos(theta) ) * ( yo + d * Math.tan(theta) );
                    var vi = Math.sqrt( numerator / denominator );
                    console.log(vi);
                    if ( ! isNaN(vi) ) {
                        
                        this.vel.x = ( vi * Math.cos(theta) ) * ( ( this.target.x - this.origin.x ) < 0 ? -1 : 1);
                        this.vel.y = -( vi * Math.sin(theta) );
